...
-- Fly/Noclip butonlar
local flyBtn=makeButton("Fly: Kapalı",contentY) contentY=contentY+52
local noclipBtn=makeButton("Duvardan Geçme: Kapalı",contentY) contentY=contentY+52

-- Fly speed slider
local flyLabel,flySliderBG,flySliderFill=makeSlider("Fly Hızı:",contentY,flySpeed,Color3.fromRGB(120,120,255)) contentY=contentY+34

-- Walk speed slider
local walkLabel,walkSliderBG,walkSliderFill=makeSlider("Walk Speed:",contentY,walkSpeed,Color3.fromRGB(120,255,120)) contentY=contentY+34
...
local function startFly()
	...
	RunService:BindToRenderStep("FroxFly",200,function()
...
flyBtn.MouseButton1Click:Connect(function()
	if flying then stopFly() flyBtn.Text="Fly: Kapalı"
	else startFly() flyBtn.Text="Fly: Açık" end
end)
...
flySliderBG.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 then draggingFly=true end end)
UserInputService.InputChanged:Connect(function(input)
...
UserInputService.InputEnded:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 then draggingFly=false end end)

-- WalkSpeed slider
local draggingWalk=false
walkSliderBG.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 then draggingWalk=true end end)
UserInputService.InputChanged:Connect(function(input)
...
UserInputService.InputEnded:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 then draggingWalk=false end end)

-- Spawn sonrası
player.CharacterAdded:Connect(function() wait(0.2) updateWalkSpeed() if flying then pcall(stopFly) pcall(startFly) end end)
